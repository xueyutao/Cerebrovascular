...
def info_preprocess(data):
    ...
    if 'dn_体重' in data.columns:
        ...
        data.loc[data['dn_体重'] >= 170, 'dn_体重'] = None
        data.loc[data['dn_体重'].isna().values == True, 'dn_体重'] = 0
    if 'dn_身高' in data.columns:
        data['dn_身高'] = np.where(data['dn_身高'] > 3, data['dn_身高'] / 100, data['dn_身高'])  # 身高异常值处理，不能低于0和大于10（m）
        data['dn_身高'] = np.where(data['dn_身高'] > 3, data['dn_身高'] / 10, data['dn_身高'])  # 身高异常值处理，不能低于0和大于10（m）
        data['dn_身高'] = np.where(data['dn_身高'] <= 1, 0, data['dn_身高'])
        data.loc[data['dn_身高'].isna().values == True, 'dn_身高'] = 0
    if '_体重指数' in data.columns:
        data['_体重指数'] = data['_体重指数'].astype(float)
        # data.loc[data['_体重指数'] <= 10] = None
        data.loc[data['_体重指数'] >= 50] = None
        data['_体重指数'] = np.where((data['_体重指数'] <= 5) & (data['dn_身高'] != 0), data['dn_体重'] / (data['dn_身高'] ** 2), data['_体重指数'])
        # data['_体重指数'] = np.where(data['_体重指数'] >= 50, data['dn_体重'] / (data['dn_身高']**2), data['_体重指数'])
        data.loc[data['_体重指数'].isna().values == True, '_体重指数'] = 0
...
def disease_preprocess():
    ...
    if 'CABG史（冠脉搭桥术）' in data.columns:
        ...
        data.loc[data['CABG史（冠脉搭桥术）'].isna().values == True, 'CABG史（冠脉搭桥术）'] = -1
    if 'dt_PCI史' in data.columns:
        ...
        data.loc[data['dt_PCI史'].isna().values == True, 'dt_PCI史'] = -1
    if 'dt_冠心病家族史' in data.columns:
        ...
        data.loc[data['dt_冠心病家族史'].isna().values == True, 'dt_冠心病家族史'] = -1
    ...
    if 'dt_肾功能不全史' in data.columns:
        ...
        data.loc[data['dt_肾功能不全史'].isna().values == True, 'dt_肾功能不全史'] = -1
    if 'dt_肿瘤病史' in data.columns:
        ...
        data.loc[data['dt_肿瘤病史'].isna().values == True, 'dt_肿瘤病史'] = -1
    if 'dt_脑血管疾病史' in data.columns:
        data.loc[data['dt_脑血管疾病史'] == '是', 'dt_脑血管疾病史'] = 1
        data.loc[data['dt_脑血管疾病史'] == '否', 'dt_脑血管疾病史'] = 0
        data.loc[data['dt_脑血管疾病史'] == '3', 'dt_脑血管疾病史'] = 1
        data.loc[data['dt_消化道疾病史'] == '2', 'dt_消化道疾病史'] = 1
        data.loc[data['dt_脑血管疾病史'].isna().values == True, 'dt_脑血管疾病史'] = -1
    if 'dt_血脂异常' in data.columns:
        ...
        data.loc[data['dt_血脂异常'].isna().values == True, 'dt_血脂异常'] = -1
    ...
    if 'dt_高血压病' in data.columns:
        ...
        data.loc[data['dt_高血压病'].isna().values == True, 'dt_高血压病'] = -1
    if 'dt_心肌梗塞史' in data.columns:
        ...
        data.loc[data['dt_心肌梗塞史'].isna().values == True, 'dt_心肌梗塞史'] = -1
    data.to_csv(out_src, header=True, index=False, encoding='utf_8_sig')
    data2 = data.loc[:,
...
def bloodtest_preprocess():
    ...
    if 'dn_NTproBNP' in data.columns:
        ...
        data.loc[data['dn_NTproBNP'].isna().values == True, 'dn_NTproBNP'] = -1
    if 'dn_中性粒细胞数' in data.columns:
        ...
        data.loc[data['dn_中性粒细胞数'].isna().values == True, 'dn_中性粒细胞数'] = -1
    ...
    if 'dn_淋巴细胞数' in data.columns:
        ...
        data.loc[data['dn_淋巴细胞数'].isna().values == True, 'dn_淋巴细胞数'] = -1
    if 'dn_甘油三酯' in data.columns:
        x = data['dn_甘油三酯'].mean()
        data.loc[data['dn_甘油三酯'].isna().values == True, 'dn_甘油三酯'] = -1
    if 'dn_白细胞总数' in data.columns:
        x = data['dn_白细胞总数'].mean()
        data.loc[data['dn_白细胞总数'].isna().values == True, 'dn_白细胞总数'] = -1
    if 'dn_直接胆红素' in data.columns:
        x = data['dn_直接胆红素'].mean()
        data.loc[data['dn_直接胆红素'].isna().values == True, 'dn_直接胆红素'] = -1
    if 'dn_空腹血糖' in data.columns:
        x = data['dn_空腹血糖'].mean()
        data.loc[data['dn_空腹血糖'].isna().values == True, 'dn_空腹血糖'] = -1
    if 'dn_糖基化血红蛋白' in data.columns:
        x = data['dn_糖基化血红蛋白'].mean()
        data.loc[data['dn_糖基化血红蛋白'].isna().values == True, 'dn_糖基化血红蛋白'] = -1
    if 'dn_红细胞体积分布宽度' in data.columns:
        x = data['dn_红细胞体积分布宽度'].mean()
        data.loc[data['dn_红细胞体积分布宽度'].isna().values == True, 'dn_红细胞体积分布宽度'] = -1
    if 'dn_红细胞平均体积' in data.columns:
        x = data['dn_红细胞平均体积'].mean()
        data.loc[data['dn_红细胞平均体积'].isna().values == True, 'dn_红细胞平均体积'] = -1
    if 'dn_红细胞总数' in data.columns:
        x = data['dn_红细胞总数'].mean()
        data.loc[data['dn_红细胞总数'].isna().values == True, 'dn_红细胞总数'] = -1
    if 'dn_红细胞比积' in data.columns:
        x = data['dn_红细胞比积'].mean()
        data.loc[data['dn_红细胞比积'].isna().values == True, 'dn_红细胞比积'] = -1
    if 'dn_肌酐' in data.columns:
        x = data['dn_肌酐'].mean()
        data.loc[data['dn_肌酐'].isna().values == True, 'dn_肌酐'] = -1
    if 'dn_肌酸激酶' in data.columns:
        ...
        x = data['dn_肌酸激酶'].mean()
        data.loc[data['dn_肌酸激酶'].isna().values == True, 'dn_肌酸激酶'] = -1
    if 'dn_肌酸激酶同工酶' in data.columns:
        x = data['dn_肌酸激酶同工酶'].mean()
        data.loc[data['dn_肌酸激酶同工酶'].isna().values == True, 'dn_肌酸激酶同工酶'] = -1
    if 'dn_肌钙蛋白I' in data.columns:
        x = data['dn_肌钙蛋白I'].mean()
        data.loc[data['dn_肌钙蛋白I'].isna().values == True, 'dn_肌钙蛋白I'] = -1
    if 'dn_肌钙蛋白T' in data.columns:
        x = data['dn_肌钙蛋白T'].mean()
        data.loc[data['dn_肌钙蛋白T'].isna().values == True, 'dn_肌钙蛋白T'] = -1
    if 'dn_血小板分布宽度' in data.columns:
        x = data['dn_血小板分布宽度'].mean()
        data.loc[data['dn_血小板分布宽度'].isna().values == True, 'dn_血小板分布宽度'] = -1
    if 'dn_血小板总数' in data.columns:
        x = data['dn_血小板总数'].mean()
        data.loc[data['dn_血小板总数'].isna().values == True, 'dn_血小板总数'] = -1
    if 'dn_血小板比容' in data.columns:
        x = data['dn_血小板比容'].mean()
        data.loc[data['dn_血小板比容'].isna().values == True, 'dn_血小板比容'] = -1
    if 'dn_血清总蛋白' in data.columns:
        x = data['dn_血清总蛋白'].mean()
        data.loc[data['dn_血清总蛋白'].isna().values == True, 'dn_血清总蛋白'] = -1
    if 'dn_血清白蛋白' in data.columns:
        x = data['dn_血清白蛋白'].mean()
        data.loc[data['dn_血清白蛋白'].isna().values == True, 'dn_血清白蛋白'] = -1
    if 'dn_血红蛋白' in data.columns:
        x = data['dn_血红蛋白'].mean()
        data.loc[data['dn_血红蛋白'].isna().values == True, 'dn_血红蛋白'] = -1
    if 'dn_谷丙转氨酶' in data.columns:
        x = data['dn_谷丙转氨酶'].mean()
        data.loc[data['dn_谷丙转氨酶'].isna().values == True, 'dn_谷丙转氨酶'] = -1
    if 'dn_超敏C反应蛋白' in data.columns:
        x = data['dn_超敏C反应蛋白'].mean()
        data.loc[data['dn_超敏C反应蛋白'].isna().values == True, 'dn_超敏C反应蛋白'] = -1
    if 'dn_载脂蛋白A' in data.columns:
        x = data['dn_载脂蛋白A'].mean()
        data.loc[data['dn_载脂蛋白A'].isna().values == True, 'dn_载脂蛋白A'] = -1
    if 'dn_载脂蛋白B' in data.columns:
        x = data['dn_载脂蛋白B'].mean()
        data.loc[data['dn_载脂蛋白B'].isna().values == True, 'dn_载脂蛋白B'] = -1
    if 'dn_间接胆红素' in data.columns:
        x = data['dn_间接胆红素'].mean()
        data.loc[data['dn_间接胆红素'].isna().values == True, 'dn_间接胆红素'] = -1
    if 'dn_高密度脂蛋白' in data.columns:
        x = data['dn_高密度脂蛋白'].mean()
        data.loc[data['dn_高密度脂蛋白'].isna().values == True, 'dn_高密度脂蛋白'] = -1
...
def medicine_preprocess():
    ...
    if 'dt_ACEI' in data.columns:
        ...
        data.loc[data['dt_ACEI'].isna().values == True, 'dt_ACEI'] = -1
    if 'dt_ADP受体拮抗剂' in data.columns:
        for i in data['dt_ADP受体拮抗剂'].unique():
            if (i == '无') | (i == '否'):
                data.loc[data['dt_ADP受体拮抗剂'] == i, 'dt_ADP受体拮抗剂'] = 0
            elif i != i:
                data.loc[data['dt_ADP受体拮抗剂'].isna().values == True, 'dt_ADP受体拮抗剂'] = -1
            else:
    ...
    if 'dt_ARB' in data.columns:
        for i in data['dt_ARB'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_ARB'].isna().values == True, 'dt_ARB'] = -1
            else:
    ...
    if 'dt_GP2b3a抑制剂' in data.columns:
        for i in data['dt_GP2b3a抑制剂'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_GP2b3a抑制剂'].isna().values == True, 'dt_GP2b3a抑制剂'] = -1
            else:
    ...
    if 'dt_β受体阻断剂' in data.columns:
        for i in data['dt_β受体阻断剂'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_β受体阻断剂'].isna().values == True, 'dt_β受体阻断剂'] = -1
            else:
    ...
    if 'dt_他汀' in data.columns:
        for i in data['dt_他汀'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_他汀'].isna().values == True, 'dt_他汀'] = -1
            else:
    ...
    if 'dt_依折麦布' in data.columns:
        for i in data['dt_依折麦布'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_依折麦布'].isna().values == True, 'dt_依折麦布'] = -1
            else:
    ...
    if 'dt_口服抗凝药' in data.columns:
        for i in data['dt_口服抗凝药'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_口服抗凝药'].isna().values == True, 'dt_口服抗凝药'] = -1
            else:
    ...
    if 'dt_抗凝药物' in data.columns:
        for i in data['dt_抗凝药物'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_抗凝药物'].isna().values == True, 'dt_抗凝药物'] = -1
            else:
    ...
    if 'dt_溶栓药' in data.columns:
        for i in data['dt_溶栓药'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_溶栓药'].isna().values == True, 'dt_溶栓药'] = -1
            else:
    ...
    if 'dt_螺内酯' in data.columns:
        for i in data['dt_螺内酯'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_螺内酯'].isna().values == True, 'dt_螺内酯'] = -1
            else:
    ...
    if 'dt_钙离子拮抗剂' in data.columns:
        for i in data['dt_钙离子拮抗剂'].unique():
            if (i == '无') | (i == '否'):
                ...
                data.loc[data['dt_钙离子拮抗剂'].isna().values == True, 'dt_钙离子拮抗剂'] = -1
            else:
    ...
    if 'dt_阿司匹林' in data.columns:
        ...
        data.loc[data['dt_阿司匹林'].isna().values == True, 'dt_阿司匹林'] = -1
...
def heartindex_preprocess():
    ...
    if 'dn_室间隔厚度' in data.columns:
        ...
        data.loc[data['dn_室间隔厚度'].isna().values == True, 'dn_室间隔厚度'] = -1
    if 'dn_左室射血分数' in data.columns:
        ...
        data.loc[data['dn_左室射血分数'].isna().values == True, 'dn_左室射血分数'] = -1
    if 'dn_左室收缩末期内径' in data.columns:
        ...
        data.loc[data['dn_左室收缩末期内径'].isna().values == True, 'dn_左室收缩末期内径'] = -1
    if 'dn_左室舒张末期内径' in data.columns:
        ...
        data.loc[data['dn_左室舒张末期内径'].isna().values == True, 'dn_左室舒张末期内径'] = -1
    if 'dn_左室重量' in data.columns:
        data.loc[data['dn_左室重量'] < 0, 'dn_左室重量'] = 0
        data.loc[data['dn_左室重量'] >= 1000, 'dn_左室重量'] = 1000
        x = data['dn_左室重量'].mean()
        data.loc[data['dn_左室重量'].isna().values == True, 'dn_左室重量'] = -1
    if 'dn_左室重量指数' in data.columns:
        ...
        data.loc[data['dn_左室重量指数'].isna().values == True, 'dn_左室重量指数'] = -1
    if 'dn_左房内径' in data.columns:
        data.loc[data['dn_左房内径'] < 0, 'dn_左房内径'] = 0
        data.loc[data['dn_左房内径'] >= 1000, 'dn_左房内径'] = 1000
        x = data['dn_左房内径'].mean()
        data.loc[data['dn_左房内径'].isna().values == True, 'dn_左房内径'] = -1
    if 'dn_短轴缩短率' in data.columns:
        ...
        data.loc[data['dn_短轴缩短率'].isna().values == True, 'dn_短轴缩短率'] = -1
